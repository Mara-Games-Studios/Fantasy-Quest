...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        public VertexPath(
            ...
            : this(
                bezierPath,
                VertexPathUtility.SplitBezierPathByAngleError(
                    bezierPath,
                    maxAngleError,
                    minVertexDst,
                    VertexPath.accuracy
        ...
        public VertexPath(BezierPath bezierPath, Transform transform, float vertexSpacing)
            : this(
                bezierPath,
                VertexPathUtility.SplitBezierPathEvenly(
                    bezierPath,
                    Mathf.Max(vertexSpacing, minVertexSpacing),
                    VertexPath.accuracy
                ),
                transform
            ) { }
        ...
        private VertexPath(
            BezierPath bezierPath,
            VertexPathUtility.PathSplitData pathSplitData,
            Transform transform
        )
        {
            ...
            for (int i = 0; i < localPoints.Length; i++)
            {
                ...
                if (space == PathSpace.xyz)
                {
                    if (i == 0)
                    {
                        localNormals[0] = Vector3
                            .Cross(lastRotationAxis, pathSplitData.tangents[0])
                            .normalized;
                    }
                    else
                    {
                        ...
                        Vector3 r =
                            lastRotationAxis
                            - (offset * 2 / sqrDst * Vector3.Dot(offset, lastRotationAxis));
                        Vector3 t =
                            localTangents[i - 1]
                ...
                {
                    localNormals[i] =
                        Vector3.Cross(localTangents[i], up) * (bezierPath.FlipNormals ? 1 : -1);
                }
            }
            ...
            if (space == PathSpace.xyz && isClosedLoop)
            {
                ...
                float normalsAngleErrorAcrossJoin = Vector3.SignedAngle(
                    localNormals[^1],
                    localNormals[0],
                    localTangents[0]
                );
            ...
            if (space == PathSpace.xyz)
            {
                for (
                    int anchorIndex = 0;
                    anchorIndex < pathSplitData.anchorVertexMap.Count - 1;
                    anchorIndex++
                )
                {
                    int nextAnchorIndex = isClosedLoop
                        ? (anchorIndex + 1) % bezierPath.NumSegments
                        : anchorIndex + 1;

                    float startAngle =
                        bezierPath.GetAnchorNormalAngle(anchorIndex)
                        + bezierPath.GlobalNormalsAngle;
                    float endAngle =
                        bezierPath.GetAnchorNormalAngle(nextAnchorIndex)
                        + bezierPath.GlobalNormalsAngle;
                    ...
                    for (int i = 0; i < num; i++)
                    {
                        ...
                        localNormals[vertIndex] =
                            rot * localNormals[vertIndex] * (bezierPath.FlipNormals ? -1 : 1);
                    }
                }
            }
        }
        ...
        )
        {
            ...
            Vector3 dir = Vector3.Lerp(
                localTangents[data.previousIndex],
                localTangents[data.nextIndex],
                data.percentBetweenIndices
            );
        ...
        )
        {
            ...
            Vector3 normal = Vector3.Lerp(
                localNormals[data.previousIndex],
                localNormals[data.nextIndex],
                data.percentBetweenIndices
            );
        ...
        )
        {
            ...
            return Quaternion.LookRotation(
                MathUtility.TransformDirection(direction, transform, space),
        ...
        public Vector3 GetClosestPointOnPath(Vector3 worldPoint)
        {
            ...
            Vector3 localResult = Vector3.Lerp(
                localPoints[data.previousIndex],
                localPoints[data.nextIndex],
                data.percentBetweenIndices
            );
        ...
        public float GetClosestTimeOnPath(Vector3 worldPoint)
        {
            ...
            TimeOnPathData data = CalculateClosestPointOnPathData(localPoint);
            return Mathf.Lerp(
                times[data.previousIndex],
                times[data.nextIndex],
                data.percentBetweenIndices
            );
        }
        ...
        public float GetClosestDistanceAlongPath(Vector3 worldPoint)
        {
            ...
            TimeOnPathData data = CalculateClosestPointOnPathData(localPoint);
            return Mathf.Lerp(
                cumulativeLengthAtEachVertex[data.previousIndex],
                cumulativeLengthAtEachVertex[data.nextIndex],
                data.percentBetweenIndices
            );
        }
        ...
        private TimeOnPathData CalculatePercentOnPathData(
            float t,
        ...
        private TimeOnPathData CalculateClosestPointOnPathData(Vector3 localPoint)
        {
            ...
            for (int i = 0; i < localPoints.Length; i++)
            {
                ...
                Vector3 closestPointOnSegment = MathUtility.ClosestPointOnLineSegment(
                    localPoint,
                    localPoints[i],
                    localPoints[nextI]
                );
            ...
            float closestSegmentLength = (
                localPoints[closestSegmentIndexA] - localPoints[closestSegmentIndexB]
            ).magnitude;
            float t =
                (closestPoint - localPoints[closestSegmentIndexA]).magnitude / closestSegmentLength;
            return new TimeOnPathData(closestSegmentIndexA, closestSegmentIndexB, t);
        }

        public readonly struct TimeOnPathData
        {

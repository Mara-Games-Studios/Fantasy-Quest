...
namespace PathCreationEditor
{
    public static class PathHandle
    {
        ...
        )
        {
            ...
            float dstToHandle = HandleUtility.DistanceToCircle(
                position,
                handleRadius + extraInputRadius
            );
                    ...
                    case EventType.MouseDown:
                        ...
                        )
                        {
                            if (dstToHandle == 0 && dstToMouse < dstMouseToDragPointStart)
                            {
                                ...
                                handleDragMouseEnd = handleDragMouseStart = Event
                                    .current
                                    .mousePosition;
                        ...
                        if (GUIUtility.hotControl == id && Event.current.button == 0)
                        {
                            ...
                            Vector3 position2 =
                                Camera.current.WorldToScreenPoint(
                                    Handles.matrix.MultiplyPoint(handleDragWorldStart)
            ...
            switch (eventType)
            {
                case EventType.Repaint:
                    ...
                    capFunc(
                        id,
                        screenPosition,
                        Quaternion.LookRotation(lookForward),
                        handleDiameter,
                        EventType.Repaint
                    );
                ...
                case EventType.Layout:
                    ...
                    HandleUtility.AddControl(
                        id,
                        HandleUtility.DistanceToCircle(screenPosition, handleDiameter / 2f)
                    );
                    Handles.matrix = cachedMatrix;
                    break;
            }

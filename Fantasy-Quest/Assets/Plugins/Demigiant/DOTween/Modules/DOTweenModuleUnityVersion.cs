...
namespace DG.Tweening
{
    ...
    public static class DOTweenModuleUnityVersion
    {
            ...
            for (int i = 0; i < len; ++i)
            {
                ...
                float colorDuration =
                    i == len - 1
                        ? duration - s.Duration(false) // Verifies that total duration is correct
                        : duration * (i == 0 ? c.time : c.time - colors[i - 1].time);
                _ = s.Append(target.DOColor(c.color, property, colorDuration).SetEase(Ease.Linear));
            }
            _ = s.SetTarget(target);
            return s;
        }
        ...
        /// </summary>
        /// <param name="elapsedLoops">Elapsed loops to wait for</param>
        public static async System.Threading.Tasks.Task AsyncWaitForElapsedLoops(
            this Tween t,
            int elapsedLoops
        )
        {
        ...
        /// </summary>
        /// <param name="position">Position (loops included, delays excluded) to wait for</param>
        public static async System.Threading.Tasks.Task AsyncWaitForPosition(
            this Tween t,
            float position
        )
        {

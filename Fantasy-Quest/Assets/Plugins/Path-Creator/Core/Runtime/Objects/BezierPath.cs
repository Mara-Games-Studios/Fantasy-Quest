...
namespace PathCreation
{
    ...
    public class BezierPath
    {
        ...
        #region Constructors
        public BezierPath()
            : this(Vector3.zero, false, PathSpace.xyz) { }
        ...
        public void AddSegmentToEnd(Vector3 anchorPos)
        {
            ...
            Vector3 secondControlForOldLastAnchorOffset =
                points[lastAnchorIndex] - points[lastAnchorIndex - 1];
            if (controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic)
            {
                ...
                secondControlForOldLastAnchorOffset =
                    (points[lastAnchorIndex] - points[lastAnchorIndex - 1]).normalized
                    * dstPrevToNewAnchor
                    * .5f;
            }
            Vector3 secondControlForOldLastAnchor =
                points[lastAnchorIndex] + secondControlForOldLastAnchorOffset;
        ...
        public void AddSegmentToStart(Vector3 anchorPos)
        {
            ...
            Vector3 secondControlForOldFirstAnchor =
                points[0] + secondControlForOldFirstAnchorOffset;
        ...
        public void SplitSegment(Vector3 anchorPos, int segmentIndex, float splitTime)
        {
            ...
            if (controlMode == ControlMode.Automatic)
            {
                points.InsertRange(
                    (segmentIndex * 3) + 2,
                    new Vector3[] { Vector3.zero, anchorPos, Vector3.zero }
                );
            ...
            else
            {
                ...
                Vector3[][] splitSegment = CubicBezierUtility.SplitCurve(
                ...
                if (controlMode == ControlMode.Mirrored)
                {
                    float avgDst =
                        (
                            (splitSegment[0][2] - anchorPos).magnitude
                            + (splitSegment[1][1] - anchorPos).magnitude
                        ) / 2;
                    MovePoint(
                        newAnchorIndex + 1,
                        anchorPos + ((splitSegment[1][1] - anchorPos).normalized * avgDst),
                        true
                    );
                }
            }
            ...
            if (isAnchorPoint || controlMode != ControlMode.Automatic)
            {
                ...
                else
                {
                    ...
                    else if (controlMode != ControlMode.Free)
                    {
                        ...
                        if (
                            (attachedControlIndex >= 0 && attachedControlIndex < points.Count)
                            || isClosed
                        )
                        {
                            ...
                            if (controlMode == ControlMode.Aligned)
                            {
                                distanceFromAnchor = (
                                    ...
                                    - points[LoopIndex(attachedControlIndex)]
                                ).magnitude;
                            }
                            ...
                            {
                                distanceFromAnchor = (
                                    points[LoopIndex(anchorIndex)] - points[i]
                                ).magnitude;
                            }
                            Vector3 dir = (points[LoopIndex(anchorIndex)] - pointPos).normalized;
                            points[LoopIndex(attachedControlIndex)] =
                                points[LoopIndex(anchorIndex)] + (dir * distanceFromAnchor);
                        }
                    }
                }
        ...
        public Bounds CalculateBoundsWithTransform(Transform transform)
        {
            ...
            for (int i = 0; i < NumSegments; i++)
            {
                ...
                List<float> extremePointTimes = CubicBezierUtility.ExtremePointTimes(
                    p[0],
                    p[1],
                    p[2],
                    p[3]
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if ((controlIndex >= 0 && controlIndex < points.Count) || isClosed)
                {
                    points[LoopIndex(controlIndex)] =
                        anchorPos + (dir * neighbourDistances[i] * autoControlLength);
                }
            }
        }
            ...
            if (isClosed)
            {
                ...
                if (NumAnchorPoints == 2)
                {
                    ...
                    Vector3 perp = Vector3.Cross(
                        dirAnchorAToB,
                        (space == PathSpace.xy) ? Vector3.forward : Vector3.up
                    );
        ...
        private void UpdateClosedState()
        {
            if (isClosed)
            {
                ...
                Vector3 lastAnchorSecondControl = (points[^1] * 2) - points[^2];
                ...
                if (controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic)
                {
                    ...
                    float dstBetweenStartAndEndAnchors = (points[^1] - points[0]).magnitude;
                    ...
                    firstAnchorSecondControl =
                        points[0]
                        + ((points[0] - points[1]).normalized * dstBetweenStartAndEndAnchors * .5f);
                }

    ...
    public static class VertexPathUtility
    {
        ...
        )
        {
            ...
            splitData.tangents.Add(
                CubicBezierUtility
                    .EvaluateCurveDerivative(bezierPath.GetPointsInSegment(0), 0)
                    .normalized
            );
            ...
            for (int segmentIndex = 0; segmentIndex < bezierPath.NumSegments; segmentIndex++)
            {
                ...
                {
                    bool isLastPointOnPath =
                        t + increment > 1 && segmentIndex == bezierPath.NumSegments - 1;
                    ...
                    if (
                        (angleError > maxAngleError && dstSinceLastVertex >= minVertexDst)
                        || isLastPointOnPath
                    )
                    {
                        ...
                        splitData.tangents.Add(
                            CubicBezierUtility.EvaluateCurveDerivative(segmentPoints, t).normalized
                        );
        ...
        public static PathSplitData SplitBezierPathEvenly(
            BezierPath bezierPath,
            float spacing,
            float accuracy
        )

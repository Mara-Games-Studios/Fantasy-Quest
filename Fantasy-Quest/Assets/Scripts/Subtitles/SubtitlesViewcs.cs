...
namespace SubtitlesSystem
{
    ...
    public class SubtitlesView: MonoBehaviour, ISubtitlesView
    {
        ...
        public void Show(string text, float timeToShow = float.MinValue, float delayAfterSaid = float.MinValue)
        {
            string curText = "";
            if (timeToShow.Equals(float.MinValue)) { timeToShow = defaultTimeSpeedForASymbol * text.Length;}
